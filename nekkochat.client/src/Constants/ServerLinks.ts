import MessageSchema from "../Schemas/MessageSchema"
 class ServerLinks {
     ...
     public static getCreateChatUrl(message: MessageSchema) {
         const url = `chats/chat/create/${message.chat_id}?value=${message.text}&sender_id=${message.sender_id}&receiver_id=${message.receiver_id}`
         return `${this.ServerUrl}${url}`;
     };
     public static getSendMessageUrl(message: MessageSchema) {
        const url = `chats/chat/send/${message.chat_id}?value=${message.text}&sender_id=${message.sender_id}&receiver_id=${message.receiver_id}`
        return `${this.ServerUrl}${url}`;
     };
